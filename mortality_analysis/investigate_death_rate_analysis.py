...
class DeathRateAnalyze:
    ...
    def analyze_and_visualize_death_rate(self):
        ...
        self.population_data['Death_Rate'] = self.population_data['Total Deaths'] / self.population_data['Total_Population']

        # Group by jurisdiction and calculate average death rate
        death_rate_by_state = self.population_data.groupby('Jurisdiction')['Death_Rate'].mean().reset_index()
        ...
        sns.scatterplot(x='Total_Population', y='Death_Rate', data=self.population_data.nlargest(10, 'Death_Rate'),
                        hue='Jurisdiction', palette='viridis', edgecolor='w', s=100)
